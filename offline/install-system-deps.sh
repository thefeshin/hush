...
detect_target_bundle() {
  ...
  PKGS_ALL_DIR="$BUNDLE_DIR/pkgs/all"
  CHECKSUM_FILE="$BUNDLE_DIR/SHA256SUMS"
}

verify_bundle_layout() {
  [[ -d "$BUNDLE_DIR" ]] || fail "Missing bundle directory: $BUNDLE_DIR"
  [[ -f "$BUNDLE_DIR/hush-offline-bundle.tar" ]] || fail "Missing Docker image tar: $BUNDLE_DIR/hush-offline-bundle.tar"
...
verify_checksums() {
  echo "[1/5] Verifying bundle checksums for $TARGET_ID..."
  (
    cd "$BUNDLE_DIR"
...
install_local_debs() {
  ...
  echo ""
  echo "[2/5] Installing local .deb packages (no network)..."
  if ! $SUDO apt-get install -y --no-download "${DEB_FILES[@]}"; then
    install_failed=1
    echo "[WARN] apt local install incomplete; attempting dpkg + offline apt fix"
  fi
