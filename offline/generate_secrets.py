#!/usr/bin/env python3
"""
Standalone secret generator for air-gapped deployment.
No external dependencies - BIP39 wordlist embedded.
"""
...
def generate_words():
    """Generate 12 random words from BIP39 wordlist using CSPRNG"""
...
def write_env(env_path, secrets_dict, config):
    """Write .env file"""
    content = f"""# HUSH Vault Configuration
# Generated at deployment - DO NOT EDIT MANUALLY

# Authentication (server-side only)
AUTH_HASH={secrets_dict['auth_hash']}
KDF_SALT={secrets_dict['kdf_salt']}
...
def main():
    """Generate all secrets and write .env file"""
    # Generate secrets
    words = generate_words()
    secrets_dict = {
        'words': words,
        'kdf_salt': generate_kdf_salt(),
        'auth_hash': generate_auth_hash(words),
    ...
    # Determine project root (parent of offline directory)
    script_dir = Path(__file__).parent
    project_root = script_dir.parent
    env_path = project_root / '.env'

    # Write .env
    write_env(env_path, secrets_dict, config)

    # Return words for display (print each word on separate line)
    for word in words:
        print(word)
